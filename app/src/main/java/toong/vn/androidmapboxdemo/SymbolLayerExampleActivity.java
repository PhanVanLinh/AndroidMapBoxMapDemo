...
public class SymbolLayerExampleActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private void setupMakiLayer() {
        mapboxMap.addLayer(new SymbolLayer(MAKI_LAYER_ID, SOURCE_ID).withProperties(
        ...
        */
        ...
        /* when feature is in selected state, grow icon */
                iconSize(property(PROPERTY_SELECTED,
                        categorical(stop(true, iconSize(1.5f)), stop(false, iconSize(1.0f)))))));
    }
    ...
    private void setupLoadingLayer() {
        mapboxMap.addLayerBelow(new CircleLayer(LOADING_LAYER_ID, SOURCE_ID).withProperties(
                circleRadius(property(PROPERTY_LOADING_PROGRESS,
                        exponential(getLoadingAnimationStops()))), circleColor(Color.RED),
                circleOpacity(0.6f)).withFilter(eq(PROPERTY_LOADING, true)), MAKI_LAYER_ID);
    }
    ...
    private void setupCalloutLayer() {
        mapboxMap.addLayer(new SymbolLayer(CALLOUT_LAYER_ID, SOURCE_ID).withProperties(
        ...
        mapboxMap.setOnMapClickListener(new MapboxMap.OnMapClickListener() {
            ...
            public void onMapClick(@NonNull LatLng point) {
                ...
                List<Feature> features =
                        mapboxMap.queryRenderedFeatures(screenPoint, CALLOUT_LAYER_ID);
                if (!features.isEmpty()) {
                    // we received a click event on the callout layer
                    Feature feature = features.get(0);
                    PointF symbolScreenPoint =
                            mapboxMap.getProjection().toScreenLocation(convertToLatLng(feature));
    ...
    private void handleClickCallout(Feature feature, PointF screenPoint, PointF symbolScreenPoint) {
        ...
        if (hitRectText.contains((int) screenPoint.x, (int) screenPoint.y)) {
            ...
            for (int i = 0; i < featureList.size(); i++) {
                if (featureList.get(i)
                        .getStringProperty(PROPERTY_TITLE)
                        .equals(feature.getStringProperty(PROPERTY_TITLE))) {
                }
            }
        }
    }
    ...
    /**
     * Set a feature selected state with the ability to scroll the RecycleViewer to the provided
     * index.
     *
    ...
    private void loadMapillaryData(Feature feature) {
        ...
        loadMapillaryDataTask =
                new LoadMapillaryDataTask(this, mapboxMap, Picasso.with(getApplicationContext()),
                        new Handler(), feature);
     ...
     */
    public void setImageGenResults(HashMap<String, View> viewMap,
    ...
    private static class LoadPoiDataTask extends AsyncTask<Void, Void, FeatureCollection> {
        ...
        protected FeatureCollection doInBackground(Void... params) {
            ...
            return new GsonBuilder().registerTypeAdapter(Geometry.class, new GeometryDeserializer())
                    .registerTypeAdapter(Position.class, new PositionDeserializer())
                    .create()
                    .fromJson(geoJson, FeatureCollection.class);
        }
     ...
     * </p>
     * <p>
     * Generating Views on background thread since we are not going to be adding them to the view
     * hierarchy.
     * </p>
     */
            ...
            if (activity != null) {
                ...
                for (Feature feature : featureCollection.getFeatures()) {
                    ...
                    imageView.setImageResource(
                            favourite ? R.drawable.ic_favorite : R.drawable.ic_favorite_border);
     ...
     */
    private static class LoadMapillaryDataTask
            extends AsyncTask<Integer, Void, MapillaryDataLoadResult> {

        static final String URL_IMAGE_PLACEHOLDER =
        ...
        public LoadMapillaryDataTask(SymbolLayerExampleActivity activity, MapboxMap map,
                Picasso picasso, Handler progressHandler, Feature feature) {
            this.activityRef = new WeakReference<>(activity);
            this.map = map;
        ...
        protected MapillaryDataLoadResult doInBackground(Integer... radius) {
            ...
            try {
                ...
                @SuppressLint("DefaultLocale") Request request = new Request.Builder().url(
                        String.format(API_URL, poiPosition.getLongitude(),
                                poiPosition.getLatitude(), poiPosition.getLongitude(),
                                poiPosition.getLatitude(), radius[0])).build();

                Response response = okHttpClient.newCall(request).execute();
                FeatureCollection featureCollection =
                        FeatureCollection.fromJson(response.body().string());
                MapillaryDataLoadResult mapillaryDataLoadResult =
                        new MapillaryDataLoadResult(featureCollection);
                for (Feature feature : featureCollection.getFeatures()) {
        ...
        protected void onPostExecute(MapillaryDataLoadResult mapillaryDataLoadResult) {
            ...
            if (mapillaryDataLoadResult == null) {
                SymbolLayerExampleActivity activity = activityRef.get();
                if (activity != null) {
                    Toast.makeText(activity, "Error. Unable to load Mapillary data.",
            ...
            FeatureCollection featureCollection =
                    mapillaryDataLoadResult.mapillaryFeatureCollection;
            ...
            if (mapillarySource == null) {
                map.addSource(new GeoJsonSource(ID_SOURCE, featureCollection,
                ...
                map.addLayerBelow(new SymbolLayer(ID_LAYER_UNCLUSTERED, ID_SOURCE).withProperties(
                        iconImage(TOKEN_UNIQUE_FEATURE), iconAllowOverlap(true), iconSize(
                                zoom(exponential(stop(18, iconSize(1.7f)), stop(17, iconSize(1.4f)),
                                        stop(16, iconSize(1.1f)), stop(15, iconSize(0.8f)),
                                        stop(12, iconSize(0.0f)))))), MAKI_LAYER_ID);

                // clustered
                int[][] layers = new int[][] {
                        new int[] { 20, Color.RED }, new int[] { 10, Color.BLUE },
                ...
                for (int i = 0; i < layers.length; i++) {
                    ...
                    clusterLayer.setProperties(circleColor(layers[i][1]), circleRadius(
                            zoom(exponential(stop(16, circleRadius(20f)),
                                    stop(15, circleRadius(18f)), stop(14, circleRadius(16f)),
                                    stop(12, circleRadius(10f))))), circleOpacity(0.6f));

                    // Add a filter to the cluster layer that hides the circles based on "point_count"
                    clusterLayer.setFilter(i == 0 ? gte("point_count", layers[i][0])
                            : all(gte("point_count", layers[i][0]),
                                    lt("point_count", layers[i - 1][0])));
                    map.addLayerBelow(clusterLayer, MAKI_LAYER_ID);
                }

                //Add the count labels
                SymbolLayer count = new SymbolLayer("count", ID_SOURCE);
                count.setProperties(textField("{point_count}"), textSize(8f),
                        textOffset(new Float[] { 0.0f, 0.0f }), textColor(Color.WHITE),
    ...
    private static class MapillaryTiles {
        ...
        static final String URL_TILESET =
        ...
        static Layer createLineLayer() {
            LineLayer lineLayer =
                    new LineLayer(MapillaryTiles.ID_LINE_LAYER, MapillaryTiles.ID_SOURCE);
            lineLayer.setSourceLayer("mapillary-sequences");
            lineLayer.setProperties(lineCap(Property.LINE_CAP_ROUND),
                    lineJoin(Property.LINE_JOIN_ROUND), lineOpacity(0.6f), lineWidth(2.0f),
